...
export async function getAllCreators(limit = 5000): Promise<CreatorEntity[]> {
  const data = await apiGet<PaginatedCreators>(`/api/v1/getAllCreators?page=1&limit=${limit}`, {
    revalidate: 1800,
  });
...
export async function getAllBrands(): Promise<BrandEntity[]> {
  const data = await apiGet<{ brands?: BrandEntity[]; data?: BrandEntity[] }>("/api/v1/allBrands", {
    revalidate: 1800,
  });
